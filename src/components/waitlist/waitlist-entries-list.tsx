...
export function WaitlistEntriesList({ entries }: WaitlistEntriesListProps) {
  ...
  const handleLeave = async (entityId: string, entryId: string) => {
    setLoadingId(entryId);
    const result = await leaveWaitlist(entityId);
    setLoadingId(null);
  ...
  return (
    <div className="space-y-4">
      {visibleEntries.map((entry) => {
        // Support both provider and daycare (legacy)
        const entity = entry.provider || entry.daycare;
        const entityId = entry.providerId || entry.daycareId;
              ...
              <AlertDialogContent>
                <AlertDialogHeader>
                  ...
                  <AlertDialogDescription>
                    ...
                    <strong>{entity.name}</strong>? You&apos;ll lose your
                ...
                <AlertDialogFooter>
                  ...
                  <AlertDialogAction
                    onClick={() => handleLeave(entityId, entry.id)}
